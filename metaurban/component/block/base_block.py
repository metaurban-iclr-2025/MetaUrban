        ...
        for x in network_block.graph.keys():
            for y in network_block.graph[x].keys():
                for i in range(3):
                    ...
                    # Plot the rectangle
                    plt.plot(*zip(*np.append(rectangle_points, [rectangle_points[0]], axis=0)), marker='o', label='['+x+']'+'['+y+']'+'['+str(i)+']', c=npy.random.rand(1, 3))

                    # Fill the rectangle with light opacity
                    plt.fill(
                        *zip(*np.append(rectangle_points, [rectangle_points[0]], axis=0)), alpha=0.3, c=npy.random.rand(1, 3)
                    )
