import logging
from metaurban.obs.image_obs import ImageStateObservation
...
from metaurban.component.sensors.distance_detector import LaneLineDetector, SideDetector
from metaurban.component.sensors.lidar import Lidar
from metaurban.constants import RENDER_MODE_NONE, DEFAULT_AGENT
from metaurban.constants import RENDER_MODE_ONSCREEN, RENDER_MODE_OFFSCREEN
from metaurban.constants import TerminationState, TerrainProperty
...
from metaurban.engine.logger import get_logger, set_log_level
from metaurban.manager.agent_manager import VehicleAgentManager
# from metaurban.manager.record_manager import RecordManager
# from metaurban.manager.replay_manager import ReplayManager
...
class BaseEnv(gym.Env):
    ...
    def __init__(self, config: dict = None):
        ...
        merged_config = self.default_config().update(config, True, ["agent_configs", "sensors"])
    ...
    def _post_process_config(self, config):
        ...
        self.logger.info("MetaUrban version: {}".format(VERSION))
    ...
    def reset(self, seed: Union[None, int] = None):
        ...
        if not self.config["sequential_seeding"]:
            self._reset_global_seed(seed)
        else:
            self._reset_global_seed_sequentially(seed)
        if self.engine is None:
            raise ValueError(
                "Current MetaUrban instance is broken. Please make sure there is only one active MetaUrban "
                ...
                "singleton of MetaUrban and restart your program."
            )
    ...
    def force_close(self):
        ...
        raise KeyboardInterrupt("'Esc' is pressed. MetaUrban exits now.")
    ...
    def _reset_global_seed_sequentially(self, force_seed=None):
        ...
        if self.current_seed is not None:
            self.seed((self.current_seed + 1) % self.num_scenarios + self.start_index)
        elif force_seed is not None:
            force_seed = (force_seed - self.start_index) % self.num_scenarios + self.start_index
            self.seed(force_seed)
        else:
            self.seed(self.start_index)
